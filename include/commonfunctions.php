...
function DisplayMap($params)
{
	...
	$pageObject->googleMapCfg['mapsData'][$params['id']]['сurrentLocationIcon'] = $params['сurrentLocationIcon'] ? $params['сurrentLocationIcon'] : '';
...
function checkTableName($shortTName )
{
	...
	if ("atendimento_agendado" == $shortTName )
		return true;
	if ("dias_disponiveis" == $shortTName )
		return true;
	if ("contas_a_pagar" == $shortTName )
		return true;
	if ("contas_pagas" == $shortTName )
		return true;
	if ("paciente" == $shortTName )
		return true;
	if ("fornecedores" == $shortTName )
		return true;
	if ("produtos1" == $shortTName )
		return true;
	if ("consultorio_odontologico_usuarios" == $shortTName )
		return true;
	return false;
}
...
function GetTablesList($pdfMode = false)
{
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Atendimento_Agendado");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="Atendimento_Agendado";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Dias_Disponiveis");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="Dias_Disponiveis";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Contas_a_pagar");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Contas_pagas");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Paciente");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Consultorio_Odontologico_usuarios");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="Consultorio_Odontologico_usuarios";
	}
	return $arr;
}
...
function GetUserPermissionsStatic( $table )
{
	...
	$extraPerm = "";
	if( $table=="Atendimento_Agendado" )
	{
		if( $sUserGroup=="Clientes" )
		{
...
function GetBaseScriptsForPage($isDisplayLoading, $additionalScripts = "", $customText = "")
{
	...
	$result = "";
	$result .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/loadfirst.js?37620")."\"></script>";


	$result .= $additionalScripts;
	$result .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/lang/".getLangFileName(mlang_getcurrentlang()).".js?37620")."\"></script>";
