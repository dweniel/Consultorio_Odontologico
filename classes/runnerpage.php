...
class RunnerPage
{
					...
					if( $dPermission["search"] )
					{
						$this->jsSettings['tableSettings'][$this->tName]['detailTables'][ $this->allDetailsTablesArr[$i]['dDataSourceTable'] ] =
							array(
								...
								'title'=> $this->getPageTitle( $this->allDetailsTablesArr[$i]['dType'], GoodFieldName( $dTable ) ),
	...
	function assignBodyEnd($params = "")
	{
		...
		echo "<script language=\"JavaScript\" src=\"".GetRootPathForResources("include/runnerJS/RunnerAll.js?37620")."\"></script>\r\n";
	 ...
	 */
	 protected function assignSearchControl() {
		...
		if( $this->searchClauseObj->isUsedSrch() || strlen( $params["simpleSrch"] ) ) {
...
//		$this->AddJSFile('libs/js/migrationTool.js');
	...
	protected function getPageTitleTemplate( $page, $table, $pSet )
	{
		...
		$templ = "";
		if( array_key_exists($table, $page_titles) && array_key_exists($page, $page_titles[ $table ][ mlang_getcurrentlang() ]) )
		{
