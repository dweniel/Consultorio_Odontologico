...
//Нет же смысла передавать ключ в открытом виде, поэтому он всегда зашифрован(по идее)
...
/*class SAMLSecurityPlugin {
	protected $sigKey = null;
	protected $encKey = null;

	public function __construct() {
		//should be replaced with keys/keys_paths
		$this->sigKey = file_get_contents('key.key');
		$this->encKey = file_get_contents('pkey.key');
	}

	public function verifyResponse($raw) {
		$res = new SAMLResponse($raw);
		return $res->verify($this->sigKey, $this->encKey);
	}
...
$plugin = new SAMLSecurityPlugin();

if ($plugin->verifyResponse(file_get_contents('file.xml'))) {
	echo "ok";
} else {
	echo "not ok";
}
