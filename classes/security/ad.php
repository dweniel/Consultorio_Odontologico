...
class SecurityPluginAd extends SecurityPlugin {
	...
	function createUserSession( $userInfo, $autoLogin = false ) {
		...
		if( $this->useDbGroups ) {
			...
			$data = Security::fetchUpdateDatabaseUser( $userData, true );
	...
	function getGroupList( $search ) {
		...
		$data = $this->ldapObj->runner_ldap_getData($filter, $this->ldapBaseDN);
	...
	protected function getMembersResult( &$data )
	{
		...
		while( $i < sizeof($data) )
		{
				$nameAttr = $this->getAttrValue( $data[$i], $this->ldapMemberAttrMap["name"] );
				if( !$nameAttr )
				{
					continue;
				}
				$result[] = array(
					...
					"displayname" => $this->ldap_getCN( $this->processAttrValue( $this->getAttrValue( $data[$i], $this->ldapMemberAttrMap["displayname"] ) ) ),
					"category" => $this->ldap_getCN( $this->processAttrValue( $this->getAttrValue( $data[$i], $this->ldapMemberAttrMap["category"] ) ) ),
					"email" => $this->processAttrValue( $this->getAttrValue( $data[$i], $this->ldapMemberAttrMap["email"] ) )
				);
			$i++;
		}
