...
class LoginPage extends RunnerPage
{
	...
	protected function logInHardcoded( $username, $password )
	{
		...
		if( Security::verifyHardcodedLogin( Security::hardcodedUsername(), Security::hardcodedPassword() ) )
		{
	...
	public function LoginWithSP( $plugin, $token, $addNewUser = true )
	{
		...
		$data = Security::fetchUpdateDatabaseUser( $info, $addNewUser );
		if( !$data ) {
			return false;
		}
	...
	protected function assignBody()
	{
		...
		$this->body["end"] .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/runnerJS/RunnerAll.js?37620")."\"></script>";
