...
//				$this->strLabel = $params['name'];
	...
	protected function isProjectDB()
	{
		...
		if("Atendimento_Agendado" == $this->chrt_array['tables'][0])
			return true;
		if("Dias_Disponiveis" == $this->chrt_array['tables'][0])
			return true;
		if("Contas_a_pagar" == $this->chrt_array['tables'][0])
			return true;
		if("Contas_pagas" == $this->chrt_array['tables'][0])
			return true;
		if("Paciente" == $this->chrt_array['tables'][0])
			return true;
		if("Fornecedores" == $this->chrt_array['tables'][0])
			return true;
		if("Produtos" == $this->chrt_array['tables'][0])
			return true;
		if("Consultorio_Odontologico_usuarios" == $this->chrt_array['tables'][0])
			return true;
		return false;
	}
