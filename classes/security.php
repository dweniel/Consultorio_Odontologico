...
class Security
{
	...
	public static function getUserGroups()
	{
		...
		$sql = "select ". $grConnection->addFieldWrappers( "Label" )
			." from ". $grConnection->addTableWrappers( "Consultorio_Odontologicouggroups" ) . " WHERE " . $grConnection->addFieldWrappers( "GroupID" )
	...
	*  For AD it is <provider code><username>
	...
	public static function _staticRestrictedPages( $table ) {
		...
		if( $group == "Clientes" )	{
			if( $table == "Consultorio_Odontologico_usuarios" )
			{
				$ret = array();
				$ret[ "search" ] = true;
				return $ret;
	...
	public static function getUgMembersDatasource() {
		...
		global $cman;
		return getTableDataSource( "Consultorio_Odontologicougmembers", $cman->getUserGroupsConnId() );
	}
	...
	public static function getUgGroupsDatasource() {
		...
		global $cman;
		return getTableDataSource( "Consultorio_Odontologicouggroups", $cman->getUserGroupsConnId() );
	}
	...
	public static function getUgRightsDatasource() {
		...
		global $cman;
		return getTableDataSource( "Consultorio_Odontologicougrights", $cman->getUserGroupsConnId() );
	}
	...
	public static function & twoFactorSettings() {
		...
		$ret["twoFactorField"] = $db["twoFactorField"];
		$ret["codeField"] = $db["codeField"];
		$ret["phoneField"] = $db["phoneField"];
		return $ret;
	}
	...
	public static function  fillTablesOwnerId( $data, $provider ) {
		...
		if( $securityType == stAD && !$provider["useDbGroups"] ) {
			...
			storageSet( "_Atendimento_Agendado_OwnerID", $userId );
						storageSet( "_Dias_Disponiveis_OwnerID", $userId );
						storageSet( "_Contas_a_pagar_OwnerID", $userId );
						storageSet( "_Contas_pagas_OwnerID", $userId );
						storageSet( "_Paciente_OwnerID", $userId );
						storageSet( "_Fornecedores_OwnerID", $userId );
						storageSet( "_Produtos_OwnerID", $userId );
						storageSet( "_Consultorio_Odontologico_usuarios_OwnerID", $userId );
		} else {
						storageSet( "OwnerID", $data["ID"] );
			storageSet( "_Atendimento_Agendado_OwnerID", $data["ID"] );
						storageSet( "_Dias_Disponiveis_OwnerID", $data["ID"] );
						storageSet( "_Contas_a_pagar_OwnerID", $data["ID"] );
						storageSet( "_Contas_pagas_OwnerID", $data["ID"] );
						storageSet( "_Paciente_OwnerID", $data["ID"] );
						storageSet( "_Fornecedores_OwnerID", $data["ID"] );
						storageSet( "_Produtos_OwnerID", $data["ID"] );
						storageSet( "_Consultorio_Odontologico_usuarios_OwnerID", $data["ID"] );
		}
	}
	 ...
	 * @return DsCommand
	 */
	protected static function createFetchUserCommand( $username, $loginControls = array(), $ignoreActivation = false, $providerCode = '' ) {
		$provider = Security::findProvider( $providerCode );
		$fieldName = ( !$providerCode || $provider["type"] == stDB )
...
//		$dc->values[ $cUserNameField ] = $info["id"];
		...
		return $dataSource->insertSingle( $dc );
	}
	...
	static function tryLoginAutoAd() {
		...
		return $plugin->loginAuto( $_SERVER["REMOTE_USER"] );
	}
	...
	/**
	* 	Return user's database record or false
	*/
	public static function fetchUpdateDatabaseUser( $userInfo, $addNewUser ) {
	   ...
	   {
		   // user fetched
		   $data =& Security::updatePluginUserData( $userInfo, $data, $dc );
	   }
	   else if( $addNewUser )
	   {
		   // insert new user
		   $data =& Security::addNewPluginUser( $userInfo );
